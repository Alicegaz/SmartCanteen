        ...
        if form.is_valid():
            ...
            for item in request.POST.getlist('quantity'):
                ...
                if int(item) !=0:
                  ...
                  w.weight = int(item)
                  w.save()
                  if Ingredient.objects.all().get(name=w.name, pk=i).weight and Ingredient.objects.all().get(name=w.name, pk=i).weight-w.weight >0:
                      ingr = Ingredient.objects.all().get(name=w.name, pk=i)
                      ingr.weight = ingr.weight - w.weight
                      ingr.save()
                  else:
                      args['error'] = "нет на складе продукта"
                      args['error'] = args['error']+w.name
                      return render(request,'blog_templates/post_new.html', args, context)
            post.save()
            form.save_m2m()
            # post.ingredients()
            return redirect('post_detail', pk=post.pk)
